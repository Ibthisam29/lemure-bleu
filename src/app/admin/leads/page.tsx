...
const STATUS_COLORS: Record<string, string> = {
  ...
  contacted: "#e3c87e",
  qualified: "#7eca8e",
  appointment_booked: "var(--champagne)",
  converted: "#a78eca",
  not_suitable: "#888",
};
      ...
      <div className="mb-8 flex items-center justify-between">
        <div>
          ...
          <h1 className="heading-display text-3xl" style={{ color: "var(--ivory)" }}>
            VIP Leads ({leads.length})
          </h1>
        </div>
        <a
          href="/api/admin/export-leads"
          className="btn-outline py-2 px-4 text-xs"
          style={{ borderColor: "rgba(184,138,114,0.4)", color: "rgba(248,243,234,0.7)" }}
        >
          Export CSV
        </a>
      </div>
        ...
        <table style={{ width: "100%", borderCollapse: "collapse" }}>
          <thead>
            <tr style={{ borderBottom: "1px solid rgba(184,138,114,0.15)" }}>
              {["Name", "Email", "Country", "Interest", "Budget", "Status", "Date", "Actions"].map((h) => (
                <th
                  key={h}
          ...
          <tbody>
            {leads.map((lead) => (
              ...
              >
                ...
                <td className="py-3 px-3 text-xs" style={{ color: "rgba(248,243,234,0.65)" }}>
                  {lead.email}
                </td>
                <td className="py-3 px-3 text-xs" style={{ color: "rgba(248,243,234,0.55)" }}>
                  {lead.country}
                </td>
                <td className="py-3 px-3 text-xs" style={{ color: "rgba(248,243,234,0.55)" }}>
                  {lead.interest_type}
                ...
                <td className="py-3 px-3">
                  <span
                    ...
                    style={{
                      background: `${STATUS_COLORS[lead.status] || "#888"}20`,
                      color: STATUS_COLORS[lead.status] || "#888",
                      border: `1px solid ${STATUS_COLORS[lead.status] || "#888"}40`,
                    }}
                  >
