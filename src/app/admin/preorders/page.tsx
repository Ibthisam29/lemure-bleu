...
const ALLOCATION_COLORS: Record<string, string> = {
  pending: "#7ec8e3",
  allocated: "#e3c87e",
  in_consultation: "var(--champagne)",
  fulfilled: "#7eca8e",
  refunded: "#888",
};

const TIER_LABELS: Record<string, string> = {
  blue_entry: "Blue Entry (SGD 300)",
  maison: "Maison (SGD 1,000)",
  legacy: "Legacy (SGD 3,000)",
};
    ...
    <div>
      <div className="mb-8 flex items-center justify-between">
        ...
        <div className="text-right">
          <p className="label-luxury" style={{ fontSize: "0.55rem", color: "rgba(184,138,114,0.6)" }}>Total Revenue</p>
          <p className="heading-display text-2xl" style={{ color: "var(--champagne)" }}>
            SGD {totalRevenue.toLocaleString()}
          </p>
        </div>
      </div>
            ...
            {preorders.map((p) => (
              <tr key={p.id} style={{ borderBottom: "1px solid rgba(255,255,255,0.04)" }}>
                ...
                <td className="py-3 px-3 text-xs" style={{ color: "rgba(248,243,234,0.6)" }}>
                  {TIER_LABELS[p.preorder_tier] || p.preorder_tier}
                </td>
                <td className="py-3 px-3 text-xs font-medium" style={{ color: "var(--champagne)" }}>
                ...
                <td className="py-3 px-3">
                  <span
                    ...
                    style={{
                      background: p.payment_status === "paid" ? "#7eca8e20" : "#88888820",
                      color: p.payment_status === "paid" ? "#7eca8e" : "#888",
                      border: `1px solid ${p.payment_status === "paid" ? "#7eca8e40" : "#88888840"}`,
                    }}
                  >
                    {p.payment_status}
                  </span>
                ...
                <td className="py-3 px-3">
                  <span
                    className="px-2 py-1 text-xs"
                    style={{
                      background: `${ALLOCATION_COLORS[p.allocation_status] || "#888"}20`,
                      color: ALLOCATION_COLORS[p.allocation_status] || "#888",
                      border: `1px solid ${ALLOCATION_COLORS[p.allocation_status] || "#888"}40`,
                    }}
                  >
