    ...
    <div>
      ...
      <div style={{ overflowX: "auto" }}>
          ...
          <tbody>
            {appointments.map(appt => (
              <tr key={appt.id} style={{ borderBottom: "1px solid rgba(255,255,255,0.04)" }}>
                ...
                <td className="py-3 px-3">
                  <p className="text-xs" style={{ color: "rgba(248,243,234,0.65)" }}>{new Date(appt.preferred_date).toLocaleDateString()}</p>
                  {appt.preferred_time && <p className="text-xs" style={{ color: "rgba(248,243,234,0.4)" }}>{appt.preferred_time}</p>}
                </td>
                <td className="py-3 px-3 text-xs" style={{ color: "rgba(248,243,234,0.6)" }}>{appt.appointment_type.replace("_", " ")}</td>
                <td className="py-3 px-3 text-xs" style={{ color: "rgba(248,243,234,0.5)" }}>{appt.budget_range}</td>
                <td className="py-3 px-3">
        ...
        {appointments.length === 0 && (
          <p className="text-center py-16 text-sm" style={{ color: "rgba(248,243,234,0.3)" }}>No appointments yet.</p>
        )}
      </div>
    </div>
  );
